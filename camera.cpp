...
{
    vec3 botLeftPos = this->film_position - this->max[0]*this->horizontal_vector - this->max[1]*this->vertical_vector;
	vec3 result = botLeftPos + this->horizontal_vector*(pixel_index[0] + 0.5)*this->pixel_size[0] + this->vertical_vector*(pixel_index[1] + 0.5)*this->pixel_size[1];
    return result;
}
