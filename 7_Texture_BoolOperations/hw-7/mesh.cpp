...
Hit Mesh::Intersect_Triangle(const Ray& ray, int tri) const
{
	...
	vec2 uv;
	if(uvs.size() > 0){
		vec2 uv_a = uvs[triangle_texture_index[tri][0]];
		vec2 uv_b = uvs[triangle_texture_index[tri][1]];
		vec2 uv_c = uvs[triangle_texture_index[tri][2]];
		uv = uv_a + beta * (uv_b - uv_a) + gamma * (uv_c - uv_a);
	}
	
	Hit hit(DBL_MAX, tri, uv);
