    ...
    const vec3& intersection_point,const vec3& normal,int recursion_depth) const
{
    ...
    for(unsigned i = 0; i < render_world.lights.size(); i++){
		...
		if(enable_acceleration)
			intersectDist = render_world.acceleration.Closest_Intersection(rayLight).second.dist;
		else
			intersectDist = render_world.Closest_Intersection(rayLight).second.dist;
		...
		if(render_world.enable_shadows && ( (intersectDist > 0) && (intersectDist <= lightVec.magnitude()) )){}
			...
			color += color_diffuse->Get_Color(hit.uv) * light->Emitted_Light(lightVec) * max(dot(normal.normalized(), lightVec.normalized()), 0.0) + 
				color_specular->Get_Color(hit.uv) * light->Emitted_Light(lightVec) * 
