    ...
    const vec3& intersection_point,const vec3& normal,int recursion_depth) const
{
    ...
    for(unsigned i = 0; i < render_world.lights.size(); i++){
		...
		if(light->name.length() == 0){ // ambient light has no name so check 0 length name
			color += color_ambient->Get_Color({}) * light->Emitted_Light(lightVec);
		} else{ // point lights DO have names
			...
			if(render_world.enable_shadows && ( (intersectDist > 0) && (intersectDist <= lightVec.magnitude()) )){}
				...
				color += color_diffuse->Get_Color({}) * light->Emitted_Light(lightVec) * max(dot(normal.normalized(), lightVec.normalized()), 0.0) + 
					color_specular->Get_Color({}) * light->Emitted_Light(lightVec) * pow(max(dot((ray.direction*-1).normalized(), (2*dot(lightVecNorm,normal.normalized())*normal.normalized()-lightVecNorm).normalized()), 0.0), specular_power);
			}
		}
    }
    return color;
